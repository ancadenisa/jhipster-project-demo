...
berries: Berry[];
    ...
    loadAll() {
        this.berryService.query().subscribe(
            (res: ResponseWrapper) => {
                this.berries = res.json;
            },
    ...
    trackId(index: number, item: Berry) {
        return item.id;
    }
    ...
    registerChangeInBerries() {
        this.eventSubscriber = this.eventManager.subscribe('berryListModification', (response) => this.loadAll());
    }
